...
#created a class for header to send in packet while transfer
class Header:
...
def stopandwait_server(iface:str, port:int, fp:BinaryIO) -> None:
    ...
    while 1:
        ...
        if recievedDgram.typeId==2:         #if typed_id is 2 i.e data recieved is correct and sending ACK
...
def stopandwait_client(host:str, port:int, fp:BinaryIO) -> None:
    ...
    data_packet=pickle.dumps(h,protocol=pickle.DEFAULT_PROTOCOL) #pickling the data for creating the header to send to server
    while msg_send:
        ...
        if responseMsg.typeId==1:
            ...
            continue
    h=Header(typeId,0,bytes())      #Done with complete file transfer now sending empty byte for end
    data_packet=pickle.dumps(h,protocol=pickle.DEFAULT_PROTOCOL)
    UDP_client.sendto(data_packet,address)
    fp.close()
